...
func copyTheme(ctx android.BaseContext) bool {
	...
	if getMakeVars(ctx, "TW_CUSTOM_THEME") == "" {
		...
		var customThemeLoc = getMakeVars(ctx, "TW_CUSTOM_THEME") + "/ui.xml"
		if _, err := os.Stat(customThemeLoc); err == nil {
			files = append(files, customThemeLoc)
		} else {
