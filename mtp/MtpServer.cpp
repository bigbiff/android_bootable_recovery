...
void MtpServer::sendDevicePropertyChanged(MtpDeviceProperty property) {
    MTPD("sendDevicePropertyChanged %d\n", property);
...
void MtpServer::sendEvent(MtpEventCode code, uint32_t param1) {
    if (mSessionOpen) {
        ...
        if (mEvent.write(mHandle))
            MTPE("Mtp send event failed: %s", strerror(errno));
    }
}
...
bool MtpServer::handleRequest() {
    ...
    MTPD("got command %s (%x)", MtpDebug::getOperationCodeName(operation), operation);
...
MtpResponseCode MtpServer::doGetPartialObject(MtpOperationCode operation) {
    ...
    const char* filePath = (const char *)pathBuf;
    MTPD("sending partial %s %" PRIu64 " %" PRIu32, filePath, offset, length);
    mtp_file_range  mfr;
    mfr.fd = open(filePath, O_RDONLY);
    if (mfr.fd < 0) {
        return MTP_RESPONSE_GENERAL_ERROR;
